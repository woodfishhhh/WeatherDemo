  ...
  <section data-testid="workspace-shortcuts"
    ...
    <div class="border-t-2 border-brand-primary/20 pt-8 flex flex-col h-full group">
      <div>
        ...
        <BilingualStack en="The strategic overview of your monitored ecosystems." zh="掌控并对比您关注的全球气象动态" wrapper-class="flex flex-col gap-4 mt-4" en-class="text-3xl md:text-5xl font-medium tracking-tighter leading-[1.1]" zh-class="text-xl font-zh-weight opacity-80" />
      ...
      <div class="border-b border-brand-primary/20 pb-8">
        ...
        <div v-if="recentLocations.length" class="flex flex-wrap gap-4">
          <button v-for="city in recentLocations" :key="city.id" type="button" data-testid="recent-location-chip"
            ...
            <BilingualStack :en="city.city" :zh="city.city" wrapper-class="flex flex-col items-start gap-1" en-class="text-sm uppercase tracking-[0.2em] font-medium" zh-class="text-xs font-zh-weight opacity-60" />
          </button>
        </div>
      ...
      <div>
        ...
        <div v-if="comparePreview.length" class="space-y-6">
          <div v-for="city in comparePreview" :key="city.id"
            ...
            <div>
              <BilingualStack :en="city.city" :zh="city.city" wrapper-class="group-hover/city:translate-x-2 transition-transform duration-500 flex items-center gap-3" en-class="text-3xl font-medium tracking-tight uppercase" zh-class="text-xl font-zh-weight opacity-60" />
            ...
            <BilingualStack en="Compare ↗" zh="分析" wrapper-class="opacity-0 group-hover/city:opacity-100 transition-opacity duration-300 flex items-center gap-2" en-class="text-xs uppercase tracking-[0.3em] font-zh-weight" zh-class="text-xs font-zh-weight opacity-60" />
          </div>
        </div>
